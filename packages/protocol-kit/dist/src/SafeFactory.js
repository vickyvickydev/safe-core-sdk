...
var _SafeFactory_instances,
  _SafeFactory_contractNetworks,
  _SafeFactory_isL1SafeSingleton,
  _SafeFactory_safeVersion,
  _SafeFactory_safeProxyFactoryContract,
  _SafeFactory_safeContract,
  _SafeFactory_provider,
  _SafeFactory_signer,
  _SafeFactory_safeProvider,
  _SafeFactory_initializeSafeFactory
...
class SafeFactory {
  ...
  async deploySafe({ safeAccountConfig, saltNonce, options, callback }) {
    ...
    const safeAddress = await __classPrivateFieldGet(
      ...
      _SafeFactory_safeProxyFactoryContract,
      'f'
    ).createProxyWithOptions({
      safeSingletonAddress: await __classPrivateFieldGet(
        this,
        _SafeFactory_safeContract,
        'f'
      ).getAddress(),
...
;(_SafeFactory_contractNetworks = new WeakMap()),
  ...
  (_SafeFactory_provider = new WeakMap()),
  (_SafeFactory_signer = new WeakMap()),
  (_SafeFactory_safeProvider = new WeakMap()),
  (_SafeFactory_instances = new WeakSet()),
  (_SafeFactory_initializeSafeFactory = async function _SafeFactory_initializeSafeFactory({
    provider,
    signer,
    safeVersion,
    isL1SafeSingleton,
