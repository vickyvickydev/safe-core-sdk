...
var _SafeProvider_instances,
  _SafeProvider_chain,
  _SafeProvider_externalProvider,
  _SafeProvider_getChain
...
class SafeProvider {
  ...
  static async init({
    ...
    if (isPasskeySigner) {
      ...
      if (!(0, provider_1.isSignerPasskeyClient)(signer)) {
        ...
        passkeySigner = await (0, utils_1.createPasskeyClient)(
          signer,
  ...
  async getExternalSigner() {
    const {
      transport,
      chain = await __classPrivateFieldGet(
        this,
        _SafeProvider_instances,
        'm',
        _SafeProvider_getChain
      ).call(this)
  ...
  async getNonce(address, blockTag) {
    return (0, actions_1.getTransactionCount)(
      __classPrivateFieldGet(this, _SafeProvider_externalProvider, 'f'),
      {
        address,
        ...(0, block_1.asBlockId)(blockTag)
  ...
  async getChainId() {
    const res = (
      await __classPrivateFieldGet(this, _SafeProvider_instances, 'm', _SafeProvider_getChain).call(
        this
      )
    ).id
    return BigInt(res)
  ...
  async getContractCode(address, blockTag) {
    const res = await (0, actions_1.getCode)(
      __classPrivateFieldGet(this, _SafeProvider_externalProvider, 'f'),
      {
        address,
        ...(0, block_1.asBlockId)(blockTag)
  ...
  async isContractDeployed(address, blockTag) {
    const contractCode = await (0, actions_1.getCode)(
      __classPrivateFieldGet(this, _SafeProvider_externalProvider, 'f'),
      {
        address,
        ...(0, block_1.asBlockId)(blockTag)
  ...
  async getStorageAt(address, position) {
    const content = await (0, actions_1.getStorageAt)(
      __classPrivateFieldGet(this, _SafeProvider_externalProvider, 'f'),
      {
        address,
  ...
  async getTransaction(transactionHash) {
    return (0, actions_1.getTransaction)(
      __classPrivateFieldGet(this, _SafeProvider_externalProvider, 'f'),
      {
  ...
  async call(transaction, blockTag) {
    const converted = (0, utils_1.toTransactionRequest)(transaction)
    const { data } = await (0, actions_1.call)(
      __classPrivateFieldGet(this, _SafeProvider_externalProvider, 'f'),
      {
        ...converted,
        ...(0, block_1.asBlockId)(blockTag)
  ...
  async readContract(args) {
    return (0, actions_1.readContract)(
      __classPrivateFieldGet(this, _SafeProvider_externalProvider, 'f'),
      args
    )
  }
  ...
  decodeParameters(types, values) {
    return (0, viem_1.decodeAbiParameters)(
      (0, viem_1.parseAbiParameters)(types),
      (0, types_1.asHex)(values)
    )
  }
}
;(_SafeProvider_chain = new WeakMap()),
  (_SafeProvider_externalProvider = new WeakMap()),
  (_SafeProvider_instances = new WeakSet()),
  (_SafeProvider_getChain = async function _SafeProvider_getChain() {
    if (__classPrivateFieldGet(this, _SafeProvider_chain, 'f'))
      return __classPrivateFieldGet(this, _SafeProvider_chain, 'f')
    const chain = (0, types_1.getChainById)(
      BigInt(await __classPrivateFieldGet(this, _SafeProvider_externalProvider, 'f').getChainId())
    )
