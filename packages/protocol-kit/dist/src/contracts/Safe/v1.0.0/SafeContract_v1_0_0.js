...
class SafeContract_v1_0_0 extends SafeBaseContract_1.default {
  ...
  async approveHash(hash, options) {
    const gasLimit =
      options?.gasLimit ||
      (await this.estimateGas('approveHash', [(0, types_1.asHash)(hash)], options))
    return (0, utils_1.toTxResult)(
      this.runner,
      await this.write('approveHash', [(0, types_1.asHash)(hash)], { ...options, gasLimit }),
      options
    )
  }
  /**
  ...
  async getModulesPaginated([start, pageSize]) {
    ...
    if ((0, utils_2.isSentinelAddress)(start)) {
      ...
      const nextPageAddress =
        nextElementIndex + size < array.length
          ? array[nextElementIndex + size]
          : constants_1.SENTINEL_ADDRESS
  ...
  async isModuleEnabled([moduleAddress]) {
    ...
    const isModuleEnabled = modules.some((enabledModuleAddress) =>
      (0, utils_2.sameString)(enabledModuleAddress, moduleAddress)
    )
