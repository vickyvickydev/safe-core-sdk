...
const createPasskeyDeploymentTransaction_1 = __importDefault(
  require('./utils/passkeys/createPasskeyDeploymentTransaction')
)
...
class Safe {
  constructor() {
    ...
    this.isValidSignature = async (messageHash, signature = '0x') => {
      const safeAddress = await this.getAddress()
      const fallbackHandler = await __classPrivateFieldGet(
        this,
        _Safe_instances,
        'm',
        _Safe_getFallbackHandlerContract
      ...
      const bytes32Tuple = [(0, types_3.asHash)(messageHash), (0, types_3.asHex)(signatureToCheck)]
      const data = fallbackHandler.encode('isValidSignature', bytes32Tuple)
      const bytesTuple = [(0, types_3.asHash)(messageHash), (0, types_3.asHex)(signatureToCheck)]
  ...
  async getAddress() {
    if (__classPrivateFieldGet(this, _Safe_predictedSafe, 'f')) {
      ...
      return (0, utils_1.predictSafeAddress)({
        safeProvider: __classPrivateFieldGet(this, _Safe_safeProvider, 'f'),
        chainId,
        isL1SafeSingleton: __classPrivateFieldGet(this, _Safe_contractManager, 'f')
          .isL1SafeSingleton,
  ...
  async getModulesPaginated(start, pageSize = 10) {
    return __classPrivateFieldGet(this, _Safe_moduleManager, 'f').getModulesPaginated(
      start,
      pageSize
    )
  }
   ...
   */
  async signHash(hash) {
    const isPasskeySigner = await __classPrivateFieldGet(
      this,
      _Safe_safeProvider,
      'f'
    ).isPasskeySigner()
    ...
    if (isPasskeySigner && signerAddress) {
      ...
      signature = await (0, utils_2.adjustVInSignature)(
    ...
    const signature = await (0, utils_2.generateSignature)(
      __classPrivateFieldGet(this, _Safe_safeProvider, 'f'),
      hash
    )
    return signature
  }
  ...
  async getOwnersWhoApprovedTx(txHash) {
    ...
    for (const owner of owners) {
      const [approved] = await __classPrivateFieldGet(
        ...
        _Safe_contractManager,
        'f'
      ).safeContract.approvedHashes([(0, types_3.asHex)(owner), (0, types_3.asHash)(txHash)])
      if (approved > 0) {
  ...
  async createAddOwnerTx(params, options) {
    ...
    if (
      isPasskey &&
      !(await __classPrivateFieldGet(this, _Safe_safeProvider, 'f').isContractDeployed(
        ownerAddress
      ))
    ) {
      ...
      const passkeyDeploymentTransaction = await (0, createPasskeyDeploymentTransaction_1.default)(
        this,
        params.passkey
      )
  ...
  async createSwapOwnerTx(params, options) {
    ...
    // The passkey Signer is a contract compliant with EIP-1271 standards, we need to check if it has been deployed.
    if (
      (0, utils_3.isNewOwnerPasskey)(params) &&
      !(await __classPrivateFieldGet(this, _Safe_safeProvider, 'f').isContractDeployed(
        newOwnerAddress
      ))
    ) {
  ...
  async executeTransaction(safeTransaction, options) {
    ...
    await __classPrivateFieldGet(this, _Safe_instances, 'm', _Safe_isReadyToExecute).call(
      this,
  ...
  async createSafeDeploymentTransaction() {
    ...
    const saltNonce =
      safeDeploymentConfig?.saltNonce || (0, utils_1.getChainSpecificDefaultSaltNonce)(chainId)
    ...
    const customContracts = __classPrivateFieldGet(this, _Safe_contractManager, 'f')
      .contractNetworks?.[chainId.toString()]
    const deploymentType = __classPrivateFieldGet(this, _Safe_predictedSafe, 'f')
      .safeDeploymentConfig?.deploymentType
    const safeSingletonContract = await (0, safeDeploymentContracts_1.getSafeContract)({
      safeProvider,
      safeVersion,
      isL1SafeSingleton,
    ...
    safeDeploymentContracts_1.getSafeProxyFactoryContract)({
      safeProvider,
      safeVersion,
      customContracts,
      deploymentType
  ...
  async createTransactionBatch(transactions, transactionOptions) {
    ...
    const multiSendCallOnlyContract = __classPrivateFieldGet(
      this,
      _Safe_contractManager,
      'f'
...
;(_Safe_predictedSafe = new WeakMap()),
  (_Safe_safeProvider = new WeakMap()),
  ...
  (_Safe_initializeProtocolKit =
    ...
    async function _Safe_initializeProtocolKit(config) {
      ...
      __classPrivateFieldSet(
        this,
        _Safe_safeProvider,
        await SafeProvider_1.default.init({
          provider,
          signer,
      ...
      if ((0, types_2.isSafeConfigWithPredictedSafe)(config)) {
        ...
        __classPrivateFieldSet(
          this,
          _Safe_contractManager,
          await contractManager_1.default.init(
            {
              provider,
              predictedSafe: __classPrivateFieldGet(this, _Safe_predictedSafe, 'f'),
              isL1SafeSingleton,
    ...
    async function _Safe_addPreValidatedSignature(transaction) {
      ...
      if (
        ...
        signedSafeTransaction.addSignature(
          (0, utils_2.generatePreValidatedSignature)(signerAddress)
        )
      }
      return signedSafeTransaction
    }),
  ...
  (_Safe_getFallbackHandlerContract =
    ...
    async function _Safe_getFallbackHandlerContract() {
      ...
      const safeVersion =
        __classPrivateFieldGet(this, _Safe_contractManager, 'f').safeContract.safeVersion ??
