...
class Safe {
  ...
  async signHash(hash) {
    ...
    if (isPasskeySigner && signerAddress) {
      ...
      signature = (0, utils_2.adjustVInSignature)(
  ...
  async getEncodedTransaction(safeTransaction) {
    ...
    const safeSingletonContract = await __classPrivateFieldGet(
      this,
      _Safe_safeProvider,
      'f'
    ).getSafeContract({
  ...
  (_Safe_initializeProtocolKit =
    ...
    async function _Safe_initializeProtocolKit(config) {
      ...
      __classPrivateFieldSet(
        this,
        _Safe_safeProvider,
        await SafeProvider_1.default.init(provider, signer, safeVersion, contractNetworks),
        'f'
      )
      __classPrivateFieldSet(
        this,
    ...
    async function _Safe_getFallbackHandlerContract() {
      ...
      const safeVersion =
        (await __classPrivateFieldGet(
          ...
          _Safe_contractManager,
          'f'
        ).safeContract.getVersion()) ?? config_1.DEFAULT_SAFE_VERSION
      const chainId = await __classPrivateFieldGet(this, _Safe_safeProvider, 'f').getChainId()
