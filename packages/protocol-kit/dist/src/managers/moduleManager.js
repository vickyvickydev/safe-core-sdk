...
class ModuleManager {
  ...
  validateModuleAddress(moduleAddress) {
    const isValidAddress = __classPrivateFieldGet(this, _ModuleManager_safeProvider, 'f').isAddress(
      moduleAddress
    )
  ...
  validateModuleIsNotEnabled(moduleAddress, modules) {
    const moduleIndex = modules.findIndex((module) =>
      (0, utils_1.sameString)(module, moduleAddress)
    )
  ...
  async getModules() {
    ...
    const [modules] = await __classPrivateFieldGet(
      this,
      _ModuleManager_safeContract,
      'f'
    ).getModules()
    return [...modules]
  }
  async getModulesPaginated(start, pageSize) {
    ...
    const [modules, next] = await __classPrivateFieldGet(
      this,
      _ModuleManager_safeContract,
      'f'
    ).getModulesPaginated([start, BigInt(pageSize)])
  ...
  async isModuleEnabled(moduleAddress) {
    ...
    const [isModuleEnabled] = await __classPrivateFieldGet(
      this,
      _ModuleManager_safeContract,
      'f'
