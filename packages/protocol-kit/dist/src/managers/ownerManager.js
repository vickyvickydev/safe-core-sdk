...
class OwnerManager {
  ...
  validateOwnerAddress(ownerAddress, errorMessage) {
    const isValidAddress = __classPrivateFieldGet(this, _OwnerManager_safeProvider, 'f').isAddress(
      ownerAddress
    )
  ...
  async getThreshold() {
    ...
    const [threshold] = await __classPrivateFieldGet(
      this,
      _OwnerManager_safeContract,
      'f'
    ).getThreshold()
  ...
  async isOwner(ownerAddress) {
    ...
    const [isOwner] = await __classPrivateFieldGet(this, _OwnerManager_safeContract, 'f').isOwner([
      ownerAddress
    ])
    return isOwner
  }
