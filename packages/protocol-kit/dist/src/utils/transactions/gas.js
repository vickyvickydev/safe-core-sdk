...
async function estimateGas(
  ...
  const safeContractContractCompatibleWithSimulateAndRevert = await (0,
  safeVersions_1.isSafeContractCompatibleWithSimulateAndRevert)(safeContract)
  const safeFunctionToEstimate = safeContractContractCompatibleWithSimulateAndRevert.encode(
    'simulateAndRevert',
    [simulateTxAccessorContract.getAddress(), (0, types_1.asHex)(transactionDataToEstimate)]
  )
...
async function estimateSafeTxGasWithRequiredTxGas(safe, safeTransaction) {
  ...
  const safeContractCompatibleWithRequiredTxGas = await (0,
  safeVersions_1.isSafeContractCompatibleWithRequiredTxGas)(safeSingletonContract)
...
async function estimateSafeTxGasWithSimulate(safe, safeTransaction) {
  ...
  const to = isSafeDeployed ? safeAddress : safeSingletonContract.getAddress()
  const SafeContractCompatibleWithSimulateAndRevert = await (0,
  safeVersions_1.isSafeContractCompatibleWithSimulateAndRevert)(safeSingletonContract)
