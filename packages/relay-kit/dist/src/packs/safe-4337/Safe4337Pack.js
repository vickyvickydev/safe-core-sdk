...
class Safe4337Pack extends RelayKitBasePack_1.RelayKitBasePack {
  ...
  static async init(initOptions) {
    ...
    if (customContracts?.entryPointAddress) {
      ...
      const supportedEntryPoints = await bundlerClient.send(
        constants_1.RPC_4337_CALLS.SUPPORTED_ENTRY_POINTS,
        []
      )
  ...
  async getEstimateFee({
    ...
    const estimateUserOperationGas = await __classPrivateFieldGet(
      ...
      _Safe4337Pack_bundlerClient,
      'f'
    ).send(constants_1.RPC_4337_CALLS.ESTIMATE_USER_OPERATION_GAS, [
  ...
  async executeTransaction({ executable }) {
    ...
    return __classPrivateFieldGet(this, _Safe4337Pack_bundlerClient, 'f').send(
      constants_1.RPC_4337_CALLS.SEND_USER_OPERATION,
      [
  ...
  async getUserOperationByHash(userOpHash) {
    return __classPrivateFieldGet(this, _Safe4337Pack_bundlerClient, 'f').send(
      constants_1.RPC_4337_CALLS.GET_USER_OPERATION_BY_HASH,
      [userOpHash]
    )
  }
  /**
  ...
  async getUserOperationReceipt(userOpHash) {
    return __classPrivateFieldGet(this, _Safe4337Pack_bundlerClient, 'f').send(
      constants_1.RPC_4337_CALLS.GET_USER_OPERATION_RECEIPT,
      [userOpHash]
    )
  }
  /**
  ...
  async getChainId() {
    return __classPrivateFieldGet(this, _Safe4337Pack_bundlerClient, 'f').send(
      constants_1.RPC_4337_CALLS.CHAIN_ID,
      []
    )
  }
}
    ...
    const safeOperation = new SafeOperation_1.default(
      {
        ...
        paymasterAndData: ethers_1.ethers.hexlify(
  ...
  (_Safe4337Pack_getSafeNonceFromEntrypoint =
    ...
    async function _Safe4337Pack_getSafeNonceFromEntrypoint(safeAddress) {
      ...
      const contract = new ethers_1.ethers.Contract(
        __classPrivateFieldGet(this, _Safe4337Pack_ENTRYPOINT_ADDRESS, 'f') || '0x',
        abi,
